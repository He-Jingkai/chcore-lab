...
int fsm_write_file(const char* path, char* buf, unsigned long size) {
        ...
        int ret = 0;
        u64 cap;
        char path_strip[FS_REQ_PATH_BUF_LEN];
        /* LAB 5 TODO BEGIN */
        {
                struct ipc_msg *ipc_msg = ipc_create_msg(fsm_ipc_struct, sizeof(struct fs_request), 0);
	        ...
	        ret = ipc_call(fsm_ipc_struct, ipc_msg);
                ...
                strcpy(path_strip, fr->getfscap.pathname);
                ipc_destroy_msg(fsm_ipc_struct, ipc_msg);
        }

        struct ipc_struct * cap_ipc_struct = ipc_register_client(cap);
        int fd = 3;
        {
                struct ipc_msg *ipc_msg = ipc_create_msg(cap_ipc_struct, sizeof(struct fs_request), 0);
                chcore_assert(ipc_msg);
                struct fs_request * fr = (struct fs_request *)ipc_get_msg_data(ipc_msg);
                fr->req = FS_REQ_OPEN;
                strcpy(fr->open.pathname, path_strip);
                fr->open.flags = O_RDONLY;
                fr->open.new_fd = fd;
                ipc_call(cap_ipc_struct, ipc_msg);
                ipc_destroy_msg(cap_ipc_struct, ipc_msg);
        }
        {
                struct ipc_msg *ipc_msg = ipc_create_msg(cap_ipc_struct, sizeof(struct fs_request) + size + 1, 0);
                chcore_assert(ipc_msg);
                struct fs_request * fr =(struct fs_request *)ipc_get_msg_data(ipc_msg);
                fr->req = FS_REQ_WRITE;
                fr->write.fd = fd;
                fr->write.count = size;
                ipc_set_msg_data(ipc_msg, buf, sizeof(struct fs_request), size + 1);
                ret = ipc_call(cap_ipc_struct, ipc_msg);
                ipc_destroy_msg(cap_ipc_struct, ipc_msg);
        }
        /* LAB 5 TODO END */
