...
static int thread_init(struct thread *thread, struct cap_group *cap_group,
  ...
  if (!thread->thread_ctx) return -ENOMEM;
...
void thread_deinit(void *thread_ptr) {
  ...
  if (thread->general_ipc_config) kfree(thread->general_ipc_config);
...
static u64 load_binary(struct cap_group *cap_group, struct vmspace *vmspace,
  ...
  if (metadata != NULL) {
    metadata->phdr_addr = elf->p_headers[0].p_vaddr + elf->header.e_phoff;
...
out_free_cap:
  for (--i; i >= 0; i--) {
    if (pmo_cap[i] != 0) cap_free(cap_group, pmo_cap[i]);
  }
out_fail:
  return r;
}
...
static int __create_root_thread(struct cap_group *cap_group, u64 stack_base,
  ...
  stack_pmo_cap = create_pmo(stack_size, PMO_ANONYM, cap_group, &stack_pmo);
  if (stack_pmo_cap < 0) {
    ret = stack_pmo_cap;
    goto out_fail;
  }
...
void test_root_thread_basic(const struct cap_group *ptr) {
  BUG_ON(ptr == NULL);
  BUG_ON(container_of(ptr, struct object, opaque)->type != TYPE_CAP_GROUP);
...
void create_root_thread(void) {
  ...
  thread_cap =
      __create_root_thread(root_cap_group, ROOT_THREAD_STACK_BASE,
                           ROOT_THREAD_STACK_SIZE, ROOT_THREAD_PRIO, TYPE_USER,
                           smp_get_cpu_id(), &__binary_root_start, ROOT_NAME);
...
int sys_create_thread(u64 thread_args_p) {
  ...
  thread_cap =
      create_thread(cap_group, args.stack, args.pc, args.arg, args.prio, type);
...
int sys_set_affinity(u64 thread_cap, s32 aff) {
  ...
  /* LAB 4 TODO END */
  if (thread_cap != -1) obj_put((void *)thread);
out:
  return ret;
}
...
s32 sys_get_affinity(u64 thread_cap) {
  ...
  if (thread == NULL) return -ECAPBILITY;
  /* LAB 4 TODO BEGIN */
