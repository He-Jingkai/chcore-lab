...
static bool is_valid_slot_id(struct slot_table *slot_table, int slot_id) {
  if (slot_id < 0 || slot_id >= slot_table->slots_size) return false;
  if (!get_bit(slot_id, slot_table->slots_bmp)) return false;
  if (slot_table->slots[slot_id] == NULL) BUG("slot NULL while bmp is not\n");
  return true;
}
...
int alloc_slot_id(struct cap_group *cap_group) {
  ...
  while (true) {
    ...
    empty_idx =
        find_next_zero_bit(slot_table->full_slots_bmp, full_bmp_size, 0);
    if (empty_idx >= full_bmp_size) goto expand;
...
int sys_create_cap_group(u64 pid, u64 cap_group_name, u64 name_len, u64 pcid) {
  ...
  if (name_len > MAX_GROUP_NAME_LEN) name_len = MAX_GROUP_NAME_LEN;
  copy_from_user(new_cap_group->cap_group_name, (char *)cap_group_name,
                 name_len);
...
struct cap_group *create_root_cap_group(char *name, size_t name_len) {
  ...
  /* LAB 3 TODO BEGIN */
  struct object_slot *obj_slot = kmalloc(sizeof(struct object_slot));
