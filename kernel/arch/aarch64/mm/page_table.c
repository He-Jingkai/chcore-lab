...
#define USER_PTE 0
#define L1_RANGE_LENGTH(len)                    \
  (((len) / (L1_PER_ENTRY_PAGES * PAGE_SIZE)) * \
   (L1_PER_ENTRY_PAGES * PAGE_SIZE))
#define L2_RANGE_LENGTH(len)                    \
  (((len) / (L2_PER_ENTRY_PAGES * PAGE_SIZE)) * \
   (L2_PER_ENTRY_PAGES * PAGE_SIZE))
#define L1_PAGE_SIZE (L1_PER_ENTRY_PAGES * PAGE_SIZE)
#define L2_PAGE_SIZE (L2_PER_ENTRY_PAGES * PAGE_SIZE)
                            ...
                            vmr_prop_t flags)
{
  ...
  if (GET_VA_OFFSET_L2(va) == 0 && GET_VA_OFFSET_L2(pa) == 0 &&
      len / (L2_PER_ENTRY_PAGES * PAGE_SIZE)) {
    for (current_ptp = (ptp_t *)pgtbl, index = 0; index <
    L2_RANGE_LENGTH(len);
