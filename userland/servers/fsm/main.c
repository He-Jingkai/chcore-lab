		...
		case FS_REQ_CREAT:
			{
				...
				struct ipc_msg *ipc_msg_redirect = ipc_create_msg(mpinfo->_fs_ipc_struct, sizeof(struct fs_request), 0);
        		struct fs_request * fr_redirect = (struct fs_request *)ipc_get_msg_data(ipc_msg_redirect);
        		fr_redirect->req = FS_REQ_CREAT;
        		strcpy(fr_redirect->creat.pathname, fr->creat.pathname);
				ret = ipc_call(mpinfo->_fs_ipc_struct, ipc_msg_redirect);
        		ipc_destroy_msg(mpinfo->_fs_ipc_struct, ipc_msg_redirect);
				break;
			}
		case FS_REQ_READ:
			{
				mpinfo = fsm_get_mount_info_withfd(client_badge, fr->read.fd);
				struct ipc_msg *ipc_msg_redirect = ipc_create_msg(mpinfo->_fs_ipc_struct, sizeof(struct fs_request) + 2 + fr->read.count, 0);
        		struct fs_request * fr_redirect = (struct fs_request *)ipc_get_msg_data(ipc_msg_redirect);
        		fr_redirect->req = FS_REQ_READ;
				fr_redirect->read.fd = fr->read.fd;
				fr_redirect->read.count = fr->read.count;
				ret = ipc_call(mpinfo->_fs_ipc_struct, ipc_msg_redirect);
				memcpy(ipc_get_msg_data(ipc_msg), ipc_get_msg_data(ipc_msg_redirect), ret);
        		ipc_destroy_msg(mpinfo->_fs_ipc_struct, ipc_msg_redirect);
				break;
			}
		case FS_REQ_GETDENTS64:
			{
				...
				struct ipc_msg *ipc_msg_redirect = ipc_create_msg(mpinfo->_fs_ipc_struct, 512, 0);
        		struct fs_request * fr_redirect = (struct fs_request *)ipc_get_msg_data(ipc_msg_redirect);
        		fr_redirect->req = FS_REQ_GETDENTS64;
				fr_redirect->getdents64.fd = fr->getdents64.fd;
				fr_redirect->getdents64.count = fr->getdents64.count;
				ret = ipc_call(mpinfo->_fs_ipc_struct, ipc_msg_redirect);
				memcpy(ipc_get_msg_data(ipc_msg), ipc_get_msg_data(ipc_msg_redirect), ret);
        		ipc_destroy_msg(mpinfo->_fs_ipc_struct, ipc_msg_redirect);
				break;
		...
		case FS_REQ_OPEN:
			{
				...
				struct ipc_msg *ipc_msg_redirect = ipc_create_msg(mpinfo->_fs_ipc_struct, sizeof(struct fs_request), 0);
        		struct fs_request * fr_redirect = (struct fs_request *)ipc_get_msg_data(ipc_msg_redirect);
        		fr_redirect->req = FS_REQ_OPEN;
        		strcpy(fr_redirect->open.pathname, fr->open.pathname);
				fr_redirect->open.mode = fr->open.mode;
				fr_redirect->open.new_fd = fr->open.new_fd;
				ret = ipc_call(mpinfo->_fs_ipc_struct, ipc_msg_redirect);
        		ipc_destroy_msg(mpinfo->_fs_ipc_struct, ipc_msg_redirect);
				break;
			}
